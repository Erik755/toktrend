...
function buildTrends() {
  ...
  // FIX: show loading state visually
  trendStatusEl.textContent = 'Analizando…';
  scanBtn.disabled = true;
  trendsEl.innerHTML = '<p class="loading-msg">Buscando tendencias…</p>';

  // Simulate async delay so loading state is visible
  setTimeout(() => {
...
function createCampaign(trend) {
  ...
  // FIX: scroll to queue after creating campaign so user sees it
...
function renderTrends() {
  ...
  if (!state.trends.length) {
    trendsEl.innerHTML = '<p class="loading-msg">Pulsa "Analizar tendencias" para comenzar.</p>';
    return;
  }
...
function renderQueue() {
  ...
  state.queue.forEach((item) => {
    ...
    // FIX: escape HTML in user-generated content to prevent XSS
    const safeTitle = item.title.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const safeHook = item.hook.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const isReady = item.status === 'Listo para publicar';
    card.innerHTML = `
      <strong>${safeTitle}</strong>
      <p>${safeHook}</p>
      <div class="meta">
        <span class="${isReady ? 'status-ready' : ''}">${item.status}</span>
      ...
      <div class="row" style="margin-top:10px">
        ...
        <button class="tiny ${isReady ? 'btn-done' : ''}" data-done="${item.id}">${isReady ? '✓ Listo' : 'Marcar listo'}</button>
        <button class="tiny btn-danger" data-delete="${item.id}">Eliminar</button>
      </div>
    `;
    queueEl.append(card);
  });
...
queueEl.addEventListener('click', async (event) => {
  ...
  if (copyId) {
    ...
    try {
      ...
      setTimeout(() => { btn.textContent = 'Copiar guion'; }, 1500);
    } catch {
      log('Error al copiar — usa HTTPS o permisos de portapapeles.');
    }
  }
  ...
  if (deleteId) {
    if (!confirm('¿Eliminar este video de la cola?')) return;
...
// ─── API Key ──────────────────────────────────────────────────────────────────
// FIX: listen to both 'input' and 'change' so key is saved as user types
...
// ─── Bottom Nav ───────────────────────────────────────────────────────────────
// FIX: each tab scrolls to its correct section
const tabSections = {
  trends: () => document.querySelector('.grid article:first-child').scrollIntoView({ behavior: 'smooth' }),
  queue: () => document.querySelector('.grid article:last-child').scrollIntoView({ behavior: 'smooth' }),
  logs: () => document.querySelector('.terminal-panel').scrollIntoView({ behavior: 'smooth' }),
  settings: () => document.querySelector('.control-panel').scrollIntoView({ behavior: 'smooth' })
};
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js')
      .then(() => log('PWA lista para instalación/offline.'))
      .catch(() => log('Service worker no disponible.'));
  });
}
